...
interface AyahCardProps {
  ...
  surahNumber: number;
  surahName: string;
  /** Ordered list of global ayah numbers (1-6236) for the whole surah */
  ...
  function handlePlay() {
    if (isThisAyahPlaying) {
      ...
      playAyah({
        surahNumber,
        surahName,
        globalAyahNumbers,          // full ordered list for this surah
    ...
    <div className={`border-b border-neutral-800 py-6 transition-colors ${isThisAyahPlaying ? "bg-neutral-900/60" : ""}`}>
      <div className="flex gap-4">
        ...
        <div className="flex shrink-0 flex-col items-center gap-3 pt-1">
          ...
          <button
            onClick={handlePlay}
            aria-label={isThisAyahPlaying ? "Pause" : "Play ayah"}
            className={`transition-colors ${isThisAyahPlaying ? "text-emerald-400 hover:text-emerald-300" : "text-neutral-500 hover:text-neutral-200"}`}
          >
            {isThisAyahPlaying ? (
